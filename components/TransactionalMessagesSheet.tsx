...
const TransactionalMessagesSheet: React.FC<TransactionalMessagesSheetProps> = ({ isVisible, onClose }) => {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      const fetchedMessages = await NotificationModule.getMessagesForDisplay();
      // Native side should already sort and limit, but re-sorting here is harmless
      // if we want to ensure a specific client-side order or if native side changes.
      fetchedMessages.sort((a, b) => b.timestamp - a.timestamp);
      setMessages(fetchedMessages);
    } catch (error) {
      console.error("Error fetching messages for display:", error);
